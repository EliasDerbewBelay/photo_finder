...
export default function PhotoGallery() {
  ...
  useEffect(() => {
    ...
    fetchPhotos();
  }, [search]); // Re-run whenever search term changes
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-8">
        {photos.map((photo) => (
          <div key={photo.id} className="overflow-hidden rounded-lg shadow-md hover:scale-105 transition-transform">
            <img 
              src={`${API_BASE_URL}/api/photo/${photo.id}/`} 
              alt={photo.file_name} 
              className="object-cover w-full h-48"
            />
            <p className="p-2 text-xs truncate text-gray-500">{photo.file_name}</p>
          </div>
        ))}
      </div>
    </div>
